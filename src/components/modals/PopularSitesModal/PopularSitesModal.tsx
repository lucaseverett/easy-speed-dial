...
export function enablePresetThumbnailsIfOff() {
  if (!settings.usePresetThumbnails) {
    settings.handleUsePresetThumbnails(true);
  }
}

const PresetThumbnailsNotice = observer(function PresetThumbnailsNotice() {
  if (settings.usePresetThumbnails) return null;
  return (
    <p className="popular-sites-notice">
      Preset thumbnails are off and will be enabled when you click Add.
    </p>
  );
});
      ...
      {sortedEntries.map(([key, preset]) => {
        ...
        return (
          ...
          >
            ...
            >
              <img src={preset.image} width="32" height="32" alt="" />
            </span>
...
interface PopularSitesPickerProps {
  selectedPresetKeys: Set<string>;
  onTogglePreset: (presetKey: string) => void;
}

export const PopularSitesPicker = observer(function PopularSitesPicker({
  selectedPresetKeys,
  onTogglePreset,
}: PopularSitesPickerProps) {
  return (
    <>
      <PresetThumbnailsNotice />
