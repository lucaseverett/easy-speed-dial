...
export const ContextMenu = observer(function ContextMenu() {
  ...
  const contextMenuX = contextMenu.coords.x;
  const contextMenuY = contextMenu.coords.y;
  ...
  useLayoutEffect(() => {
    ...
    contextMenuRef.current?.style.setProperty(
      "--context-menu-top",
      `${
        contextMenuY + contextMenuRef.current?.offsetHeight >=
        window.innerHeight
          ? contextMenuY - contextMenuRef.current?.offsetHeight
          : contextMenuY
      }px`,
    ...
    contextMenuRef.current?.style.setProperty(
      "--context-menu-left",
      `${
        contextMenuX + contextMenuRef.current?.offsetWidth >= window.innerWidth
          ? contextMenuX - contextMenuRef.current?.offsetWidth
          : contextMenuX
      }px`,
    );
  ...
  function handleMouseEnter(e: MouseEvent<HTMLButtonElement>) {
    setCurrIndex(menuItemsRef.current.indexOf(e.target as HTMLButtonElement));
  }
